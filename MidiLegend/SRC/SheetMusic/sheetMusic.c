...
SheetMusic *newSheetMusic(FILE *midiFile)
{
    ...
    //******************************************************************************************************************
    // Cr�ation des changements de tempo
    ...
    for (trackIdx = 0; trackIdx < midi->nbTracks; trackIdx++) {
        for (channelIdx = 0; channelIdx < 16; channelIdx++) {
            // Calcul du nombre de notes dans la port�e
            nbNotes = 0;
            for (i = 0; i < midi->nbNotes[trackIdx]; i++) {
                if ((midi->notes[trackIdx][i].channel == channelIdx) && (midi->notes[trackIdx][i].velocity > 0)) {
                    nbNotes++;
                }
            }
            ...
            if (noteEvt->channel == channelIdx) {
                ...
                if (noteEvt->velocity > 0) {
                    ...
                    if (curNotesOn[noteEvt->note] != -1) {
                        // La note pr�c�dente n'avait pas �t� �teinte
...
void freeSheetMusic(SheetMusic *sheet)
{
    ...
    if (sheet) {
        if (sheet->nbNotes != NULL) {
            free(sheet->nbNotes);
        }
        if (sheet->tempos != NULL) {
            free(sheet->tempos);
        }
        if (sheet->notes != NULL) {
            for (i = 0; i < sheet->nbStaves; i++) {
                if (sheet->notes[i] != NULL) {
                    free(sheet->notes[i]);
                }
            }
            free(sheet->notes);
