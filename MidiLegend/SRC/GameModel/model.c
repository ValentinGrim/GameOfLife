...
GameSheet *newGameSheet(SheetMusic *sheet)
{
    ...
    gameSheet = (GameSheet *)calloc(1, sizeof(GameSheet));
    nbStaves = sheet->nbStaves;
...
void checkStrum(Model *model)
{
    ...
    int nbNotes = *(model->gameSheet->nbNotes),
        i;           ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (i = 0; i < nbNotes; i++) // Boucle pour verifier chaque nots une par une
    {
        ...
        if (notes[i].visible == 1 && notes[i].state != statePlayed) // On verifie si la note est visible (donc en jeu) et si elle n'as pas deja été jouer
        {
            ...
            {
                if ((notes[i].playingTime <= model->timer->currentTime + 0.09) && (notes[i].playingTime >= model->timer->currentTime - 0.09)
                    && (model->keys->fretDown[string] == 1)) // La longue conditions pour verifier si la note est dans la hit box (0.1 représente la largeure
                                                             // du'une note donc 0.09 permet d'activer des que la note touche le strum)
                {
                    ...
                    {
                        model->keys->fretDown[string] = 0; // passage de l'etat de la fret a 0 une fois qu'une note a été valider si le cheat mode est
                                                           // desactiver afin d'empecher de reester appuyer)
                    }
            ...
            {
                if ((notes[i].playingTime <= model->timer->currentTime + 0.09) && (notes[i].playingTime >= model->timer->currentTime - 0.09)
                    && (model->keys->fretDown[string] == 1) && (model->keys->strumDown == 1)) {
